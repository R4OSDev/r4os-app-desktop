...
pub fn r4_app_main(r4_app: *r4os.App) i32 {
    ...
    defer if (ctx.graphics) |graphics| {
        ...
        ctx.printU64(graphics.batches);
        ctx.write(" fills=");
        ctx.printU64(graphics.fills);
        ctx.write(" rejected=");
        ctx.printU64(graphics.rejected_batches);
        ctx.write("\n");
        graphics.destroy();
        ctx.graphics = null;
    };
    if (ctx.graphics) |graphics| {
        if (graphics.info()) |info| {
            ctx.write("R4DESK gfx: DEVICE_V1 backend=");
            ctx.printU64(info.backend);
            ctx.write(" adapter=");
            ctx.printU64(info.adapter_id);
            ctx.write("\n");
        }
    ...
    defer if (composition) |worker| {
        ctx.write("R4DESK composition: visible=");
        ctx.printU64(worker.frames_visible);
        ctx.write(" uploads=");
        ctx.printU64(worker.engine.uploaded_bytes);
        ctx.write(" draws=");
        ctx.printU64(worker.engine.render_jobs);
        ctx.write(" preparations=");
        ctx.printU64(worker.prepared_threads);
        ctx.write("\n");
        worker.destroy();
    };
