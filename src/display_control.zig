...
pub const Owner = struct {
    ...
    pub fn step(self: *Owner, ctx: *api.Context, manager: *outputs.Manager, service_available: bool) bool {
        ...
        var reply: a.DisplayControlExchange = .{};
        const request: a.DisplayControlExchange = .{ .desktop_owner = ctx.self_handle, .status = self.state };
        const rc = ctx.displayControlExchange(&request, &reply);
        ...
        if (reply.request.action != 0) self.handle(ctx, manager, &reply.request, now)
        else if (!self.busy() and !self.mode.busy() and manager.active() and !manager.reconcile) self.restore(ctx, manager, now);
        return changed;
    }
    fn handle(self: *Owner, ctx: *api.Context, manager: *outputs.Manager, request: *const a.DisplayControlRequest, now: u64) void {
        ...
        switch (request.action) {
            1 => {
                ...
                self.mode = .{}; self.mode_key = null; self.mode_started = false; self.configured = false; self.confirming = false;
    ...
    fn keep(self: *Owner, manager: *outputs.Manager) void {
        if (self.desired) |desired| {
            ...
            if (!self.restoring and policy.persistent(&manager.layout)) manager.savePreferences(&manager.layout) catch {
        ...
        self.before = null; self.desired = null; self.confirming = false; self.restoring = false;
    }
